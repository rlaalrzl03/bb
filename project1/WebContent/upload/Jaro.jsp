...
<% 
JaroDAO jaroDAO = new JaroDAO();
	//전체글의 개수를보기
	int count = jaroDAO.getJaroCount();
	...
	List<JaroBean> list = null;
	
	if(count>0){
	    
	    list = jaroDAO.getJaroList(startRow, pageSize);
		...
		<div id="body" class="home">
			<div class="header">
				...
				<tr>
					...
					<td class= "Jaro">File</td>
				</tr>
				<%
				if (count>0){
				    for(int i = 0; i<list.size(); i++){
				        JaroBean jbean = list.get(i);
				    %>     
				        <tr onclick="location.href='JaroContent.jsp?num=<%=jbean.getNum()%>&pageNum=<%=pageNum%>'">
						<td><%=jbean.getNum() %></td>
						<td class="left">
						<%
						int wid =0;
						if(jbean.getRe_lev()>0){
						    wid= jbean.getRe_lev() *10;
						 %>   
						...
						<%=jbean.getSubject() %></td>
						<td><%=jbean.getName() %></td>
						<td><%=sdf.format(jbean.getDate()) %></td>
						<td><%=jbean.getReadcount() %></td>
						<td><%=jbean.getFile() %></td>
						</tr>
					<% 	
				...
				%>
				<div id = noticeButton>
				<input type = "button" value="글쓰기" class="btn" onclick="location.href='JaroWrite.jsp'">
				</div>
				<%
				}
				%>
				...
				%>
					<a href="Jaro.jsp?pageNum=<%=startPage-pageBlock%>">[이전]</a>
				<%
				}
				// [1] [2]...... 페이지번호 나타내기
				for(int i=startPage; i<=endPage; i++){
				%>
					<a href="Jaro.jsp?pageNum=<%=i%>">[<%=i%>]</a>
				<%	
				}
				//[다음] 끝페이지 번호가 전체페이지 수보다 작을떄..
				if(endPage < pageCount){
				%>	
					<a href="Jaro.jsp?pageNum=<%=startPage+pageBlock%>">[다음]</a>		
				<%
				}
			}
			%>
