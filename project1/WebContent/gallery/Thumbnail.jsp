...
<% 
thumbDAO thumbDAO = new thumbDAO();
	//전체글의 개수를보기
	int count = thumbDAO.getThumbCount();
	...
	List<thumbBean> list = null;
	
	if(count>0){
	    
	    list = thumbDAO.getThumbList(startRow ,pageSize);  
			...
			<div class="header">
				...
				if (count>0){
				    for(int i = 0; i<list.size(); i++){
				        thumbBean bean = list.get(i);
				    %>    
				        <tr onclick="location.href='ThumbContent.jsp?num=<%=bean.getNum()%>&pageNum=<%=pageNum%>'">
						<td><img src="../image/<%= bean.getFile() %>" id="images"></td>
						</tr>
						<% 
				    }//for끝
				...
				%>
				<div id = noticeButton>
				<input type = "button" value="글쓰기" class="btn" onclick="location.href='Thumbnailwrite.jsp'">
				</div>
				<%
				}
				%>
				...
				%>
					<a href="Thumbnail.jsp?pageNum=<%=startPage-pageBlock%>">[이전]</a>
				<%
				}
				// [1] [2]...... 페이지번호 나타내기
				%>
				...
				%>
					<a href="Thumbnail.jsp?pageNum=<%=i%>" >[<%=i%>]</a>
				<%	
				}
				//[다음] 끝페이지 번호가 전체페이지 수보다 작을떄..
				if(endPage < pageCount){
				%>	
					<a href="Thumbnail.jsp?pageNum=<%=startPage+pageBlock%>">[다음]</a>		
				<%
				}
			}
			%>
